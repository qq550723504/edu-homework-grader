...
本地默认 `GENERATION_PROVIDER=fake`，不会访问外部模型。当前开发阶段只使用固定、已批准的 Provider/模型 ID 和唯一的 `generator-v1` Prompt；真实 M1、M2、E1、E4 Provider 验收只在 `ai-provider-acceptance` 受保护 Environment 中运行。
...
K–12 是平台的内部层级，不是对学生地区、学校课程或能力的推断。课程目录只保存经审核的短目标摘要、稳定代码和权威来源元数据；不复制教材、试卷或课标长文本。首批有效 Profile 为中国学前发展（2012）、中国义务教育（2022）、中国普通高中（2017/2020）与 CEFR（2020），来源和治理规则见 [AI 出题计划](docs/ai-question-generation-plan.md)。
