...
export interface GenerationDefaultChange { id: string; status: string; provider_name: string; model_version: string; prompt_version: string; request_reason: string }
...
export function decideGenerationDefaultChange(request: Request, csrfToken: string, id: string, action: 'approve' | 'reject' | 'apply', reason: string): Promise<GenerationDefaultChange> {
  return request('/api/core/v1/admin/ai-generation-default-change-requests/' + encodeURIComponent(id) + '/' + action, { method: 'POST', headers: { 'X-CSRF-Token': csrfToken }, body: { reason } })
}
