...
def test_ci_reuses_cached_images_and_only_installs_python_for_browser_e2e() -> None:
    ...
    assert not any(step.get("uses") == "actions/setup-python@v5" for step in web_steps)
    assert not any(step.get("run") == "make install-python" for step in web_steps)
    ...
    assert not any(step.get("run") == "docker compose build api grader web languagetool" for step in compose_steps)
