...
from ..models import Classroom, ClassTeacher, Role, Tenant, User
from .roster import RosterRow, _import_roster_rows
    ...
    *,
    ...
    if not all(values.values()):
        raise CustomerProvisioningError(
            "tenant, operator, and teacher identity fields are required"
        )
    if not rows:
    ...
    with session.begin():
        ...
        operator_identity = session.scalar(
            select(User).where(
                User.oidc_issuer == values["oidc_issuer"],
                User.oidc_subject == values["operator_subject"],
            )
        )
        if operator_identity is not None and operator_identity.tenant_id != tenant.id:
            raise CustomerProvisioningError("operator identity belongs to another tenant")
        ...
        if operator is None:
            operator = User(
                ...
                oidc_subject=values["operator_subject"],
                display_name="Customer provisioning operator",
            )
            session.add(operator)
            session.flush()
        elif operator.role is not Role.ADMIN:
            raise CustomerProvisioningError("existing OIDC operator identity is not an admin")
        ...
        if teacher is None:
            ...
            if teacher is not None and teacher.oidc_subject != values["teacher_subject"]:
                raise CustomerProvisioningError(
                    "teacher email is already bound to another identity"
                )
        if teacher is None:
            teacher = User(
                tenant=tenant,
        ...
        imported_students = _import_roster_rows(
            session,
            CurrentPrincipal(
                ...
                oidc_subject=operator.oidc_subject or "",
            ),
            rows,
        )
