# K–12 课程约束型 AI 出题实施计划

状态：教师 AI 出题工作台已交付（#41）；质量评估、治理和集成验收仍由开放的 #42、#43、#31 跟进
更新日期：2026-07-23
...
Provider，但必须从 Secret Manager 注入 `OPENAI_API_KEY`，显式指定经过审批的
`GENERATOR_OPENAI_MODEL`，并使用专用主机 allowlist。当前官方 GPT-5.6 允许的版本化模型 ID 是
`gpt-5.6-sol`、`gpt-5.6-terra` 和 `gpt-5.6-luna`；`gpt-5.6`、`latest` 等浮动别名会被拒绝。若使用
兼容中转端点，必须将其精确主机名加入 `GENERATOR_PROVIDER_ALLOWED_HOSTS`。候选题只进入
`generated_question_drafts`，不创建或发布 `QuestionVersion`。
...
7. **K–12 是内部层级。** 必须映射到具体地区、课程标准、语言能力框架和版本。
8. **不以生成量衡量质量。** 核心指标是答案准确性、年级适配、重复率、教师接受率和发布后改答案率。

## 3. K–12 内部层级与课程 Profile
...
- M2 MathJSON 可规范化并进入安全 AST；对 Schema 有效且策略版本为 `"2"` 的候选题，Core 按固定顺序将标准答案、一单位合成偏移、空 MathJSON、零分母和受限深度 MathJSON 作为五个原始信封交给现有 Grader；任一不可用、格式异常、非有限分数或不符合接受/拒绝/复核契约的结果都会阻止候选题，持久化证据只记录稳定的探针标识，不保存题干、MathJSON、AST、Grader 反馈或异常文本；
- M2 的一单位偏移只验证候选题的确定性 Grader 一致性，不是从学习者行为推断的 common misconception；显式整理的误解语料、结果校准与黄金评估仍是 #42 工作，warning 的教师确认以及候选题接受、入库和发布仍由 #41 与既有发布门禁负责；
...
| 6–8 | 教师工作台 | 发起生成、查看进度、审核、编辑、接受和拒绝 | #41（已交付） |
| 8–10 | 评估与治理 | 黄金评估集、质量门槛、配额、审计、模型/Prompt 生命周期 | #42、#43（开放） |
| 10–11 | 集成验收 | 真实服务链路、题库发布、多题作业和故障降级 | #31（开放）、#36 |
...
截至 2026-07-23，#41 已交付：浏览器验收覆盖公开有序难度计划、G7 M1/M2 的
passed/warning 批量接受与 warning 显式确认、blocked 候选不可入库、接受后的
`QuestionVersion` 保持 `draft`，以及 G8 E1/E4 单候选重生成时题型和难度计划保持且源任务不变。
#42、#43 和 #31 仍明确保持开放，不纳入 #41 的完成范围。

| Issue | 工作流 | 优先级 |
| --- | --- | --- |
| #31 | 真实服务链路与集成验收（开放） | P1 |
| #36 | AI 出题 Epic 与整体验收 | Epic |
| #37 | K–12 课程数据模型 | P0 |
| #38 | 课程导入、审核与来源治理 | P0 |
| #39 | 生成服务和 provider 抽象 | P0 |
| #40 | 答案、难度、重复与安全门禁 | P0 |
| #41 | 教师 AI 出题工作台（已交付） | P1 |
| #42 | 离线/线上质量评估（开放） | P1 |
| #43 | 模型、Prompt、隐私、配额和版权治理（开放） | P1 |
| #44 | K 阶段非评分活动 | P2 |
