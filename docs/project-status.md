...
| Grader | M1/M2、E1–E4、受限 MathJSON、LanguageTool 和固定英语嵌入模型已实现；E3/E4 保持教师最终复核；内部 HTTP 调用已有显式超时 | 学校网络、生产环境持续运行和试点验收由 #33 负责 |
...
| 部署 | Compose、Kubernetes 清单、SHA 镜像发布工作流，以及每周 COS 备份和隔离恢复自动化已实现 | 清单和自动化存在不等于环境已部署；#33 的实际 rollout、监控、备份恢复和回滚尚未验收 |
...
CI Run #656（Run ID `30329115969`）成功，包含：
...
`Publish production images` Run `30329286394` 在同一提交上成功生成并校验 4 个镜像 digest，并在部署之前完成 `release-evidence`。证据 Artifact 的两次隔离真实服务运行共覆盖 28 个场景，清理和重复运行均成功；相似度读取超时场景确认超时后不会发起额外调用，并能在恢复后通过。

部署 Job 仍停在受保护的生产审批，**本证据不等同于生产部署、发布环境统一验收或生产已上线**。
...
| Capacity-aware production Ruleset | `rules-v9` |
| Verification capacity rules | `verification-capacity-v1` |
| Grader HTTP timeout | `10s`，可由 `GRADER_REQUEST_TIMEOUT_SECONDS` 覆盖 |
...
`verification-capacity-v1` 在题干指纹、重复查询、MathJSON 规范化和 Grader/LanguageTool/相似度调用之前执行，并限制：
...
| #29 | 英语 E1–E4 教师建题已完成。 |
| #30 | 多题、多题型作业编排已完成。 |
| #31 | 已完成：真实 Keycloak/OIDC、PostgreSQL、Provider、Grader 和浏览器统一验收。 |
...
| #43 | 开放：运营治理剩余项。 |
| #83 | 已完成：一致性、回归语料和故障门禁已由受保护 RC 的两轮、28 场景真实服务证据验证。 |
| #99 | 开放：首次真实只读数据库评估、成本完整性和发布后修正映射。 |
| #108 | 已完成：容量、总预算、细分依赖超时和性能证据已在受保护 RC 签发。 |
| #122 | 已完成：发布前 RC 门禁已绑定 source SHA 与 4 个镜像 digest；两轮真实服务证据通过，部署仍保留受保护生产审批。 |
...
```text
#99 正式生产评估 ────────┐
#43 运营治理 ────────────┼→ #42 教师阈值与线上证据 ─┐
#32 学生同步可靠性 ─────────────────────────────────┴→ #33 学校试点部署
```
