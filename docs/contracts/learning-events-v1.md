...
| `verification_status` | enum | `draft`、`pending_review`、`verified`、`rejected` |
| `verified_by_user_id` | UUID，可空 | 教师或课程管理员；仅 `verified`/`rejected` 时填写 |
| `verified_at` | timestamp，可空 | 审核时间；仅 `verified`/`rejected` 时填写 |
| `version` | integer | 乐观锁版本 |

约束：

- `context` 关系不形成掌握度证据；
- `draft`、`pending_review` 和 `rejected` 映射可以保存以支持生成或管理员录入后的教师审核，但不得进入学生模型；
- 已有 Learning Event 引用的映射不得原地修改；
- 修改映射必须创建新 revision，并只影响后续事件；
- 事件创建时必须固化当时生效的 `item_objective_links.id` 集合；后续映射 revision 不得重解释历史事件；
...
| `question_version_id` | UUID，可空 | 题目版本；作答、评分和订正事件必填，教师覆盖和隐私阻断事件可空 |
| `attempt_id` | UUID | 作答尝试，可空 |
| `attempt_answer_id` | UUID | 答案记录，可空 |
| `mapping_link_ids` | UUID array，可空 | 创建事件时生效的已审核 `item_objective_links.id` 集合；题目证据事件必填，非题目事件为空 |
| `objective_revision_id` | UUID，可空 | 教师覆盖事件的目标；`teacher_evidence_override` 必填，题目证据事件由 `mapping_link_ids` 导出 |
| `event_type` | enum | 事件类型 |
| `occurred_at` | timestamp | 业务事实发生时间 |
| `recorded_at` | timestamp | 事件写入时间 |
| `replay_sequence` | bigint | 源事务提交后由串行 Outbox relay 分配的严格单调游标 |
| `evidence_status` | enum | `eligible`、`provisional`、`excluded`；一律不可原地改写 |
| `score` | decimal | 最终或当前得分，可空 |
| `max_score` | decimal | 满分，可空 |
| `decision` | string | 判定结果，可空 |
| `response_time_ms` | integer | 可空；仅在可靠采集、用途审批已版本化且模型输入已启用时使用 |
| `hint_count` | integer，可空 | 提示次数；仅作答范围事件填写 |
| `attempt_number` | integer，可空 | 第几次作答；仅作答范围事件填写 |
...
```text
{tenant_id}:{source_type}:{source_id}:{attempt_answer_id-or-item_identity}:{event_type}:{source_version}
```

生产实现可以使用稳定哈希，但必须保证同一业务事实重复消费时得到相同键。对一个
`GradePublication` 投影出的多题事件，键必须包含 `attempt_answer_id` 或等价的题目/答案身份，
不能让不同答案共享同一个 `event_key`。
...
不能删除或修改旧事件（包括 `evidence_status` 和分数）。状态计算读取追加式当前有效事件视图：
若存在有效的后续事件以 `supersedes_event_id` 引用旧事件，则旧事件不再参与当前状态；完整重放
仍可还原任何历史水位的状态。

订正的作答记录可能属于独立的 `CorrectionAttempt`，不能只按“同一 `AttemptAnswer`”关联。每个
`correction_published` 必须以 `supersedes_event_id` 指向被订正的原始有效证据事件，并携带原始
`attempt_answer_id` 作为审计引用；状态视图只保留该替代链的最新有效事件，避免同时累计原错误和订正证据。
...
`replay_sequence` 必须在源业务事务提交后，由单写者或等价串行化的 Outbox relay 分配；时间戳、
随机 UUID、数据库普通 sequence 都不能单独作为并发提交下的重放水位。运行过程中产生的新事件
进入下一批，不与当前重放混合。
...
- 停止创建新的模型事件；
- 允许且要求追加不含题目或答案身份的 `privacy_processing_blocked` 事件，作为派生状态和推荐
  projector 的阻断/tombstone 信号；该事件始终为 `excluded`，不构成学习证据；
- 停止状态更新和推荐；
- 不删除依法或业务必须保留的原始成绩记录；
- 按数据保留政策删除、隔离或去标识化派生事件和状态；
- 取消或删除该学生的待处理 Outbox 与 Dead Letter 记录，并在 relay 侧再次校验处理权限，防止清理后重新创建事件；
- 记录不含学生敏感内容的审计事实。
...
- 同一业务事实重复投递只产生一条事件；
- 同一发布记录的多题答案生成不同 `event_key`；
- 自动判分后教师改分，旧证据被 supersede；
- 映射 revision 变更后，历史事件仍按持久化的 `mapping_link_ids` 重放；
- 订正结果形成独立事件且优先于原错误证据；
- `needs_review` 不形成强证据；
- Grader 依赖故障不改变掌握状态；
- 一题多目标按 criterion 正确拆分；
- 迟到事件触发受影响状态重算；
- 并发事务以提交后 `replay_sequence` 增量重放时不遗漏事件；
- 跨租户读取和写入被拒绝；
- 同意撤回后不再创建学习证据，但会追加一个 `privacy_processing_blocked` tombstone；
- 删除请求覆盖全部派生对象；
- 从空状态全量重放得到稳定、可重复结果。
