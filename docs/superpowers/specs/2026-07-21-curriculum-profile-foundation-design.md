...
模型借鉴 [1EdTech CASE 1.1](https://standards.1edtech.org/case/) 的稳定目标标识和显式关系思想，但本期不实现 CASE 的导入、导出或互操作 API。
...
- 拒绝自引用、跨 profile 与重复有向边；同一 profile 的依赖写入在 profile 行锁下检测循环，避免并发写入形成环。
...
首期的课程管理员能力由现有 `admin` 角色加平台 OIDC subject 白名单共同承担；当前用户模型为单角色枚举，新增可叠加权限角色会扩大身份治理范围。只有 `CURRICULUM_ADMIN_SUBJECTS` 中的 `admin` 可创建 profile、添加映射、提交修订、审核、启用、停用及维护先修关系。教师仅可读取有效目录并选择目标；学生没有课程目录端点。将来需要把课程编辑权限从平台管理员中拆分时，再以独立权限表或 OIDC claim 扩展，而不是给用户追加第二个互斥角色。
...
非法层级、题型或 profile 组合返回稳定的 HTTP 422。普通读取者访问不存在、无效或无权访问的资源返回 404；写接口沿用现有不枚举资源的鉴权约定，对非 `admin` 也返回 404。
...
GET /v1/curriculum-profiles/{profile_code}/grade-mappings
GET /v1/curriculum-profiles/{profile_code}/objectives?grade_level=&subject=&domain=&knowledge_point=&question_type=
```

平台管理员通过 `/v1/admin/curriculum/*` 写入 profile、映射、目标、revision、先修关系和生命周期动作。审核与启用动作写入审计事件，其中保存执行者、目标 revision、状态和来源版本；绝不保存学生数据或复制的来源文本。
