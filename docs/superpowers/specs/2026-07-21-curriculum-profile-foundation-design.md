...
首期的课程管理员能力由现有平台 `admin` 角色承担；当前用户模型为单角色枚举，新增可叠加权限角色会扩大身份治理范围。`admin` 可创建 profile、添加映射、提交修订、审核、启用、停用及维护先修关系。教师仅可读取有效目录并选择目标；学生没有课程目录端点。将来需要把课程编辑权限从平台管理员中拆分时，再以独立权限表或 OIDC claim 扩展，而不是给用户追加第二个互斥角色。
