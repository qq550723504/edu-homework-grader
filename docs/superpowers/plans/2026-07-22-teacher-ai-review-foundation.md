...
- Create: `apps/api/alembic/versions/0019_ai_generated_question_reviews.py`
- Modify: `apps/api/src/edu_grader_api/services/generation.py:250-278`
- Test: `apps/api/tests/test_generation_models.py`
- Test: `apps/api/tests/test_question_validation_models.py`
...
- [x] **Step 3: Implement models and portable migration**
...
ruff check apps/api/src/edu_grader_api/models.py apps/api/src/edu_grader_api/services/generation.py apps/api/alembic/versions/0019_ai_generated_question_reviews.py apps/api/tests/test_generation_models.py
ruff format --check apps/api/src/edu_grader_api/models.py apps/api/src/edu_grader_api/services/generation.py apps/api/alembic/versions/0019_ai_generated_question_reviews.py apps/api/tests/test_generation_models.py
```

- [x] **Step 5: Commit**

```powershell
git add apps/api/src/edu_grader_api/models.py apps/api/src/edu_grader_api/services/generation.py apps/api/alembic/versions/0019_ai_generated_question_reviews.py apps/api/tests/test_generation_models.py
...
- [x] **Step 1: Write failing revision-validation tests**
...
- [x] **Step 3: Implement snapshot-bound verification**
...
- Create: `apps/api/alembic/versions/0020_question_version_reading_material.py`
...
- Test: `apps/api/tests/test_assignments_api.py`
- Test: `apps/web/tests/student-workflow.test.ts`
...
- [x] **Step 3: Implement append-only review service**

Validate user edits with `GeneratedCandidate.model_validate`; reject changed objective revision, type or policy version before appending revision `N + 1`, then synchronously run Task 2 verification. Reject only with the fixed reasons `incorrect_answer`, `out_of_scope`, `unclear_wording`, `duplicate`, `unsuitable_for_students`, `other`; `other` needs 1–500 characters. If a rejection has no current validation run, create one first so every decision satisfies Task 1's required evidence FK; its status never blocks rejection. Accept locks draft, current revision and latest run, rejects missing/stale/blocked results, requires warning confirmation, invokes existing `create_question`, and creates a bounded, normalized title derived from the candidate prompt (with a deterministic fallback only when no safe text remains). E4 accepts store `candidate.reading_material` in a distinct `QuestionVersion.reading_material` field while retaining only the candidate question in `QuestionVersion.prompt`; migration `0020` adds that nullable text field, successor drafts copy it, student assignment detail projects it separately, and the student UI renders it before the question. Never concatenate the two strings, so the existing 10,000-character prompt limit and prompt-only fingerprint/de-duplication contract remain intact. Persist the decision, source version and audit event in the same transaction; never publish or create test results.
...
- [x] **Step 1: Write failing authorization and API tests**
...
- [x] **Step 3: Implement route request models and safe projections**
...
```powershell
$env:PYTHONPATH='apps/api/src;services/generator/src;packages/processor-policy/src'
python -m pytest apps/api/tests/test_generation_models.py apps/api/tests/test_question_verification.py apps/api/tests/test_ai_question_generation_api.py apps/api/tests/test_ai_question_review.py apps/api/tests/test_questions.py apps/api/tests/test_question_validation_models.py -q
ruff check apps/api/src/edu_grader_api apps/api/tests/test_generation_models.py apps/api/tests/test_question_verification.py apps/api/tests/test_ai_question_generation_api.py apps/api/tests/test_ai_question_review.py apps/api/tests/test_questions.py apps/api/tests/test_question_validation_models.py
ruff format --check apps/api/src/edu_grader_api apps/api/tests/test_generation_models.py apps/api/tests/test_question_verification.py apps/api/tests/test_ai_question_generation_api.py apps/api/tests/test_ai_question_review.py apps/api/tests/test_questions.py apps/api/tests/test_question_validation_models.py
